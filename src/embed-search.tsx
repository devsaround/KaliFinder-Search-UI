...
function getStoreUrlFromScript(): string {
  ...
  for (const script of scripts) {
    ...
    if (src && src.includes('kalifind-search')) {
      try {
        ...
        if (storeUrl) {
          console.log('[KaliFinder] Found storeUrl from script src:', storeUrl);
          return storeUrl;
        }
      } catch (error) {
        console.warn('[KaliFinder] Failed to parse script URL:', src, error);
      }
    }
  }
  ...
  if (storeUrl) {
    console.log('[KaliFinder] Found storeUrl from page URL:', storeUrl);
    return storeUrl;
  }
  ...
  console.warn('[KaliFinder] No storeUrl found, using default: http://localhost:8080');
...
function injectSearchIconIfNeeded(): void {
  ...
  // Create search icon button using the same CSS class as WidgetTrigger
  const injectedButton = document.createElement('button');
  injectedButton.id = 'kalifinder-injected-search-button';
  injectedButton.className = 'kalifinder-trigger-button';
  injectedButton.setAttribute('aria-label', 'Search');

  // SVG Search Icon (same as WidgetTrigger)
  injectedButton.innerHTML = `
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <circle cx="11" cy="11" r="8"></circle>
      <path d="m21 21-4.35-4.35"></path>
    </svg>
    <span class="kalifinder-trigger-tooltip">Powered by KaliFinder</span>
  `;

  // CRITICAL: Add click handler to open the widget
  injectedButton.onclick = () => {
    console.log('[KaliFinder] Injected button clicked, opening widget');
    openKalifinderWithQuery('');
  };
